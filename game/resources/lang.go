...
func LangTranslate(key string, args ...any) string {
	...
	if !exists {
		log.Warn(fmt.Sprintf("no translation found for key (%s)", key))
		format, exists = fallback_language_map[key]
	}
	if !exists {
		log.Warn(fmt.Sprintf("no fallback translation found for key (%s)", key))
		return key
	}
...
func append_lang(path string, m map[string]string) error {
	...
	for k, v := range lang {
		if other_v, exists := m[k]; exists {
			log.Warn(fmt.Sprintf("overlapping language keys (%s) \"%s\" and \"%s\"", k, v, other_v))
		}
		m[k] = v
	}
