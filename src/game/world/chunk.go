...
type Chunk struct {
	...
	Entities    map[ecs.EntityRef]struct{}
}
...
func CreateChunk() *Chunk {
	return &Chunk{
		IsGenerated: false,
		Blocks:      [CHUNK_VOLUME]BlockId{},
		Entities:    make(map[ecs.EntityRef]struct{}),
	}
}
...
func (c *Chunk) AddEntity(id ecs.EntityRef) error {
	if c == nil {
		return fmt.Errorf("nil chunk")
	}
...
func (c *Chunk) RemoveEntity(id ecs.EntityRef) error {
	if c == nil {
		return fmt.Errorf("nil chunk")
	}
